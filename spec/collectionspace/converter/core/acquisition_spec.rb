...
RSpec.describe CollectionSpace::Converter::Core::CoreAcquisition do
  ...
  let(:xpaths) {[
    ...
    #{ xpath: '/document/*/approvalGroupList/approvalGroup/approvalGroup', transform: ->(text) { CSURN.parse(text)[:label] } },
    #{ xpath: '/document/*/approvalGroupList/approvalGroup/approvalIndividual', transform: ->(text) { CSURN.parse(text)[:label] } },
    #{ xpath: '/document/*/approvalGroupList/approvalGroup/approvalStatus', transform: ->(text) { CSURN.parse(text)[:label] } },
