...
RSpec.describe CollectionSpace::Converter::Core::CoreObjectExit do
  ...
  let(:xpaths) {[
    ...
    { xpath: '/document/*/deacApprovalGroupList/deacApprovalGroup/deaccessionApprovalStatus', transform: ->(text) { CSURN.parse(text)[:label].downcase } },
    { xpath: '/document/*/deacApprovalGroupList/deacApprovalGroup/deaccessionApprovalGroup', transform: ->(text) { CSURN.parse(text)[:label].downcase } },
